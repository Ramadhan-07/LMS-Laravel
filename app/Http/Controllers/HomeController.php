...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $discountTab_courses = DB::table('courses')
                    ...
                    // ->orWhere('courses.strike_out_price', '<>' ,0)
                    ->get();
    ...
    public function jobList(Request $request)
    {
        ...
        $archieves = DB::table('jobs')
                ->select(DB::raw('YEAR(created_at) year, MONTH(created_at) month, MONTHNAME(created_at) month_name, COUNT(*) job_count'))
                ->groupBy('year')
                ->groupBy('month')
                ->orderBy('year', 'desc')
                ->orderBy('month', 'desc')
        ...
        return view('site.jobs.list', compact('jobs', 'archieves'));
    }

    public function jobView($job_slug = '', Request $request)
    {
        $paginate_count = 1;

        $jobs =  Job::where('is_active',1)->limit(7)->get();
        ...
        return view('site.jobs.view', compact('job', 'jobs', 'archieves'));
    }
