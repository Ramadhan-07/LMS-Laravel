...
class CourseController extends Controller
{
    ...
    public function instructorQuizCurriculum($course_id = '',Request $request)
    {
        ...
        return view('instructor.course.create_quiz', compact('course', 'video'));
    }
    ...
    public function courseView($course_slug = '', Request $request)
    {
        ...
        return view('site.course.view', compact('course', 'curriculum_sections', 'lectures_count', 'videos_count', 'video', 'course_breadcrumb', 'is_curriculum', 'course_taken', 'course_rating', 'students_count'));
    }
    ...
    {   
        ...
        $videos_count = $curriculum['videos_count'];
        $is_curriculum = $curriculum['is_curriculum'];
        $course_taken = CourseTaken::where('course_id', $course->id)->where('user_id', \Auth::user()->id)->first();
        $video = null;
        if($course->course_video)
        {
            $video = $this->model->getvideoinfoFirst($course->course_video); 
        ...
        return view('site.course.learn', compact('course', 'curriculum_sections', 'lectures_count', 'videos_count', 'video', 'course_breadcrumb', 'is_curriculum', 'course_taken', 'course_rating', 'students_count'));
    }
    ...
    public function checkout($course_slug = '', Request $request)
    {
        ...
        \Midtrans\Config::$serverKey = 'SB-Mid-server-3KmL3R8OdECVdiuvaG2RW9R0';
        ...
        if($course->price !=0)
        {
            ...
            return view('site.course.checkout', compact('course', 'course_breadcrumb'), ['snap_token'=>$snapToken]);
        }
        else{
    ...
    public function checkout_post(Request $request)
    {
        ...
        $transaction->amount = $json->gross_amount;
        $transaction->status =  $json->transaction_status;
        $transaction->payment_method = $json->payment_type;
        $transaction->save();
		...
		$credit = new Credit;
		$credit->transaction_id = $transaction;
		$credit->instructor_id = $instructor_id;
		$credit->course_id = $course->id;
		...
		$credit->save();
                ...
                return redirect(url('/'))->with ('alert-success', 'pesanan berhasil dibuat');
    ...
    public function adminCourseList(Request $request)
    {
        ...
        if($request->has('search')){
            ...
            $courses = DB::table('courses')
                        ...
                        ->where('courses.course_title', 'LIKE', '%' . $search . '%')
                        ->orWhere('courses.course_slug', 'LIKE', '%' . $search . '%')
                        ->orWhere('courses.instructors.first_name', 'LIKE', '%' . $search . '%')
                        ->orWhere('categories.name', 'LIKE', '%' . $search . '%')
                        ->paginate($paginate_count);
        }
        else {
    ...
    {   
        ...
        return view('admin.course.create_info', compact('course', 'categories', 'instructors', 'instruction_levels'));
    }
    ...
    // {
    //     Category::destroy($course_id);
    //     return $this->return_output('flash', 'success', 'Course deleted successfully', 'admin/categories', '200');
    // }
    ...
    public function instructorCourseInfoSave(Request $request)
    {
        ...
        $course->meet_link = $request->input('meet_link');
    ...
    public function instructorCourseVideoSave(Request $request)
    {
        ...
        $video_image_path = storage_path('public/'.$path.'/'.$video_image_name);
